      ...
      echo $submit_errors;
        ...
        if (!$result OR mysqli_num_rows($result) == 0) {
            ...
            if (is_null($grade)) {
                ...
                $result = mysqli_query($db, $query);
                $query2 = "UPDATE group_list SET grade=grade+$overall, num_groups=num_groups+1 WHERE id_report_group='$report_group' AND id_eval_group='$group_entry' AND class='$class'";
